...
- **Generated files are never edited by hand.** `logger/slogx/semconv_gen.go` and `temporalx/semconv_gen.go` come from `make semconv-generate`; change `semconv/registry/model/` and regenerate, in the same commit — CI diffs them.
