...
| **`obsx/`** | `SetupObservability` — the single OTel SDK wiring point (call once in `main`, defer `Shutdown`). Builds per-signal providers from `Config`/`ConfigFromEnv`: traces (OTLP/HTTP, ParentBased sampler + W3C propagator), metrics (OTLP/HTTP PeriodicReader with SLO-preserving Views — HTTP/gRPC `DurationBuckets`, byte buckets, and DB-scale `DBDurationBuckets` for the otelpgx `db.client.operation.duration` histogram, which the dbconv helper creates with no bucket hint — plus Go runtime instrumentation; on by default since the RFC-0014 P3 cutover), logs (OTLP/HTTP via the `otelzap` bridge, opt-in). Installs the enabled providers as the OTel globals so contrib instrumentation (otelgin, otelgrpc, Temporal SDK) exports with no extra wiring — there is **no Prometheus exporter, no `/metrics` endpoint, and no default-registry bridge** (the scrape-era `SetupMetrics` was removed at P3). `TraceIDFromContext` returns the active span's trace ID for log↔trace correlation; `TraceContext(ctx)` returns a `SkipType` zap field carrying ctx so the `otelzap` bridge stamps the OTLP log record with the native trace_id/span_id (stronger than a hand-added string) without the context leaking into the stdout JSON tee. |
| **`dbx/`** | Postgres pool builder pre-wired for OpenTelemetry — the single place the fleet instruments its data layer. `NewPool(ctx, dsn, opts...)` parses the DSN, applies the transaction-mode-pooler-safe settings (`SimpleProtocol` + statement/description caches off, unchanged from the per-service `Connect` it replaces), attaches an `otelpgx` query tracer with RFC-0017 safe defaults (`WithTrimSQLInSpanName` for bounded span names, `WithDisableAcquireTracer`, `WithDisableConnectionDetailsInAttributes`, and **never** `WithIncludeQueryParameters` — bind params are PII/secrets), then `RecordStats` (pgxpool.* pool-stat metrics; fail-boot on registration error) and pings. The tracer also emits `db.client.operation.duration` + `db.client.operation.errors` per operation (bounded `pgx.operation.type` label); the duration histogram gets its DB-scale buckets from obsx's `DBDurationBuckets` View — otelpgx itself ships no bucket hint. `WithMaxConns` mirrors `cfg.Database.MaxConnections`; the tracer/meter providers default to the OTel globals obsx installs (test-injection options exist). Query spans are children of the active span in ctx — a bare `context.Background()` query produces no span by otelpgx design. |
